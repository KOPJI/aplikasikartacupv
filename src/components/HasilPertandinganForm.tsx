...
const HasilPertandinganForm = () => {
  ...
  useEffect(() => {
    ...
    const selectedMatch = pertandingan.find(p => p.id === id);
    if (!selectedMatch) {
      navigate('/jadwal');
      return;
    }
  ...
  }, [id, pertandingan, getTeam, getTeamPlayers, navigate]);
              ...
              {formData.kartu.map((card, index) => (
                ...
                >
                  ...
                  <div className="w-32">
                    <select
                      value={card.jenis}
                      onChange={(e) => updateCard(index, 'jenis', e.target.value)}
                      className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white"
                    >
...
export default HasilPertandinganForm;
